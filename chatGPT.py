import openai
import genai
from genai.schemas import HarmCategory, HarmBlockThreshold
...
model = genai.GenerativeModel(
    ...
    Also make a cypher query for knowledge. Let's say you get the entity as the keyword FROM THE QUESTION and you identify the relation, the code would be:
    MATCH (e {{name: entity}})-[r:relation]->(related)
    RETURN e,r,related
    Make sure the arrow: -> is correctly pointed every time it may not be towards related sometime it can be away too.
    Give only single cypher query.
    Just give output for cypher as:
    Cypher code is:
    (the code)
    and don't include ``` this is in the answer.
    If there is no match, do not generate cypher code, just print No cypher code for the above answer.
    Now, if I ask the question in any language, 
    give the answer in that language too.
    Only for cypher: entity name will be translated to English only.
    """
)
...
def answer_question(question):
    ...
    # Parse question to find entities
    ...
    # Find relation in dataset if both entities are provided
...
while True:
    ...
    # If no response from dataset, use AI model to generate a response
